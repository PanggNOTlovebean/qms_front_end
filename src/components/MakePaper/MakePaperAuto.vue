    ...
    <Card class="card">
      <div class="form-wrapper" style="height: 100% ">
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
          ...
          <FormItem>
            <Button type="primary" @click="handleSubmit('formValidate')">随机组卷</Button>
    ...
    handleSubmit (name) {
      ...
      this.$refs[name].validate((valid) => {
        ...
        if (valid) {
          ...
          // console.log(postdata);
          this.axios.post('/makePaper', this.formValidate)
            .then(function (response) {
              sessionStorage.setItem('singlechoose',response.data.singlechoose)
              sessionStorage.setItem('multiplechoose',response.data.multiplechoose)
              sessionStorage.setItem('judge',response.data.judge)
              sessionStorage.setItem('blank',response.data.blank)
              sessionStorage.setItem('discuss',response.data.discuss)
              ...
              that.$router.push('/Manager/Paper')
            })
